...
В данном файле нет явных `#define`-макросов.
...
FreeRTOS-задача обработки дискретного ввода/вывода. Работает с периодом 10 мс. Выполняет три основные функции:
...
|--------|---------|-----|------|-----------|----------|
| Modbus | `modbus_poller_task` | `"modbus"` | 4096 байт | 6 | Циклический опрос Modbus-устройств |
| IO | `io_task` | `"io"` | 2048 байт | 6 | Обработка DI/DO, E-STOP, debounce |
| Process | `process_task` | `"process"` | 8192 байт | 5 | Основная логика управления процессом |
| Watchdog | `watchdog_task` | `"watchdog"` | 2048 байт | 7 | Контроль работоспособности |
...
|-----------|--------|-------------|
| 7 (высший) | `watchdog` | Контроль работоспособности должен выполняться без задержек |
| 6 | `modbus` | Своевременный опрос датчиков критичен для безопасности |
| 6 | `io` | Обработка E-STOP и DI/DO требует минимальной задержки |
| 5 | `process` | Логика управления может допускать небольшие задержки |
