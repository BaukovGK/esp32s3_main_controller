...
- **Интеграцию с alarm_manager** -- регистрирует callback для получения аварийных событий и помещает их в кольцевой буфер для асинхронной публикации.
...
|-----|----------|----------|
| `ALARM_QUEUE_SIZE` | `16` | Размер кольцевого буфера аварий. Определяет максимальное количество аварийных событий, которые могут быть буферизованы до обработки задачей `MqttTask`. При переполнении новые аварии отбрасываются. |

### Неявные константы

| Константа | Значение | Контекст |
|-----------|----------|----------|
| Топик LWT | `"ro_plant/availability"` | Используется как `last_will.topic` при конфигурировании MQTT-клиента |
| Сообщение LWT | `"offline"` | Публикуется брокером при разрыве соединения |
| LWT QoS | `1` | Quality of Service для LWT сообщения |
| LWT Retain | `true` | LWT сохраняется на брокере |
| Reconnect timeout | `5000` ms | Таймаут переподключения MQTT-клиента |
| Размер стека задачи | `8192` байт | Стек FreeRTOS-задачи `mqtt` |
| Приоритет задачи | `4` | Приоритет FreeRTOS-задачи `mqtt` |
| Имя задачи | `"mqtt"` | Имя FreeRTOS-задачи для отладки и диагностики |
| Частота диагностики | каждый 6-й цикл | Диагностика публикуется каждые 6 циклов основного цикла (при интервале 5с -- примерно каждые 30с) |
...
| `s_alarm_queue[]` | `alarm_entry_t [16]` | Кольцевой буфер (ring buffer) аварийных событий. Заполняется из `alarm_notify_cb` (вызывается из контекста alarm_manager), считывается в `mqtt_task`. |
| `s_alarm_queue_head` | `volatile int` | Индекс записи (голова) кольцевого буфера. Инкрементируется при добавлении аварии. |
| `s_alarm_queue_tail` | `volatile int` | Индекс чтения (хвост) кольцевого буфера. Инкрементируется при извлечении аварии для публикации. |
...
2. Читает конфигурацию MQTT из `config_manager` (URI брокера, client_id, логин, пароль, интервал публикации).
3. Конфигурирует MQTT-клиент ESP-IDF:
   - Адрес брокера.
   - Идентификатор клиента.
   - Last Will and Testament (топик `ro_plant/availability`, сообщение `"offline"`, QoS 1, retain).
   - Таймаут переподключения 5 секунд.
   - Авторизация (если username не пустой; password -- если также не пустой).
4. Создаёт MQTT-клиент (`esp_mqtt_client_init`).
5. Регистрирует обработчик событий `mqtt_event_handler` на все типы событий.
6. Регистрирует callback `alarm_notify_cb` в `alarm_manager`.
7. Запускает MQTT-клиент (неблокирующий вызов -- `esp_mqtt_client_start` создаёт внутреннюю задачу ESP-IDF).
8. Создаёт FreeRTOS-задачу `mqtt_task` ("mqtt", стек 8192, приоритет 4).
9. Регистрирует задачу в модуле диагностики.
...
**Примечание:** LWT-сообщение `"offline"` будет опубликовано брокером при разрыве TCP-соединения. Однако при штатной остановке `mqtt_publish_online` не вызывается с `"offline"` -- брокер сделает это через механизм LWT.
...
**Описание:**
Callback-функция, регистрируемая в `alarm_manager`. Вызывается при возникновении или снятии аварии. Помещает аварийное событие в кольцевой буфер `s_alarm_queue` и пробуждает `MqttTask` через `xTaskNotifyGive`.

**Параметры:**

| Параметр | Тип | Описание |
|----------|-----|----------|
| `entry` | `const alarm_entry_t *` | Указатель на структуру аварийного события. Копируется в буфер. |

**Возвращаемое значение:** нет (`void`).

**Особенности:**
- Работает в контексте вызывающего (может быть ISR или другая задача).
- При переполнении буфера (все 16 слотов заняты) -- новое событие **отбрасывается** (без уведомления).
- Пробуждает `MqttTask` даже если буфер переполнен (для обработки уже имеющихся записей).
...
5. **Публикация аварий из очереди:** последовательно извлекает и публикует все аварии из кольцевого буфера.
6. **Публикация полного статуса** (`mqtt_publish_full_status`) -- вызывается каждый цикл.
7. **Публикация диагностики** (`mqtt_publish_diagnostics`) -- каждый 6-й цикл (управляется счётчиком `diag_counter`).
...
|-------|-----|--------|----------|
| `ro_plant/availability` | 1 | Да | `"online"` при подключении, `"offline"` через LWT при обрыве |
...
| `ro_plant/status/diagnostics` | Каждый 6-й цикл |
...
**Потоки:**
- **Основной поток `MqttTask`**: периодическая публикация, обработка очереди аварий.
- **Обработчик событий MQTT** (внутренняя задача ESP-IDF): получение событий подключения/отключения/данных.
- **Callback аварий** (контекст alarm_manager): запись в кольцевой буфер.
