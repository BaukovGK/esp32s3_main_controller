...
| Что | Куда |
|---|---|
| **Bugfix-исправления** | прямо в `dev` (commit + push) |
| **Новые фичи** | новая ветка от `dev` → PR в `dev` |
| **Релиз** (когда проект полностью рабочий на стенде) | merge `dev` → `main` через PR |

**Правила:**
- `main` — релизная, защищена (PR review обязателен)
- `dev` — рабочая, текущая интеграционная точка
- Все feature-ветки именуются `feature/<short-description>`, удаляются после merge
- Каждый коммит сопровождается прогоном host-тестов (`cd test && cmake --build build && ctest --test-dir build`)
- Новые модули с публичным API покрываются хост-тестами в `test/`
...
├── modbus_poller/     Modbus RTU мастер — опрос slave-устройств (AI/расходомер/кондуктометры/счётчики KWS)
├── drivers/           Драйверы датчиков:
│   ├── analog_input   Давление/температура (4–20мА, raw в мкА → mA = raw/1000,
│   │                  ratio = (mA−4)/16, value = min + ratio×(max−min);
│   │                  sensor-fault на оба конца: <3.5мА обрыв, >20.5мА КЗ;
│   │                  скользящее среднее N=8)
│   ├── flowmeter      Расходомер УРЖ2КМ (word-swapped IEEE 754; объём — 4 канала V1..V4)
│   ├── conductivity   Кондуктометры СЛ21 (4 канала на 2 приборах:
│   │                  slave 10 → FEED+PERM1, slave 11 → PERM2+CONC)
│   └── power_meter    Счётчики KWS-306L (НД- и ВД-насосы, slave 20/21):
│                      U/I/P/E/T, реле 0x003F через FC 0x06 — TODO следующая фаза
...
|-------|------------|------------|
| 1 | Waveshare AI 8CH | 8 аналоговых входов 4–20мА (raw в мкА): давление P1–P4, температура |
| 2 | УРЖ2КМ | Ультразвуковой расходомер: 4 канала расхода Q1..Q4 + 4 канала объёма V1..V4 (V5..V8 не сверены с протоколом ТЕСС, до сверки игнорируются) |
| 10 | СЛ21-201 | Кондуктометр: FEED (X1/t1) + PERM1 (X2/t2) — питательная вода и пермеат 1-й ст. |
| 11 | СЛ21-101 | Кондуктометр: PERM2 (X1/t1) + CONC (X2/t2) — пермеат 2-й ст. (товарный) и концентрат |
| 20 | KWS-306L | Счётчик электроэнергии НД-насоса (U/I/P/E/T) |
| 21 | KWS-306L | Счётчик электроэнергии ВД-насоса (U/I/P/E/T) |
